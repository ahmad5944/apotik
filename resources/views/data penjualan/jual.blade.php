...
<form action="/datapenjualan/simpan" method="POST">
      @csrf
        ...
        <div class="form-group col-sm-4">
            ...
            <input type="hidden" name="datapenjualan" value="{{ $jual->no_jual }}" class="form-control" id="exampleFormControlInput1" >           
            @endforeach
        ...
        <div class="form-group col-sm-4">
            ...
            <input type="text" min="1" name="tgl_jual" value="{{ $pnj->tgl_jual }}" id="addnmbrg" class="form-control" id="exampleFormControlInput1" require readonly>         
        </div>
        @endforeach        
...
<table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
    ...
    <tr>
        <input name="no_jual[]" class="form-control" type="hidden" value="{{$temp->no_jual}}" readonly>
        <td><input name="kd_brg[]" class="form-control" type="hidden" value="{{$temp->kd_brg}}" readonly>{{$temp->kd_brg}}</td>
        <td><input name="nm_brg[]" class="form-control" type="hidden" value="{{$temp->nm_brg}}" readonly>{{$temp->nm_brg}}</td>
        <td><input name="qty_jual[]" class="form-control" type="hidden" value="{{$temp->qty_jual}}" readonly>{{$temp->qty_jual}}</td>
